          ...
          <el-tab-pane v-for="tabKey in ['mine', 'shared-with-me', 'shared-by-me']" :key="tabKey" :label="tabLabels[tabKey]" :name="tabKey">
            <div class="columns-grid">
              <el-checkbox
                  ...
                  :key="key"
                  v-model="formData.columns[tabKey][key]"
                  :label="label"
...
const loadSettings = () => {
  ...
  Object.keys(formData.value.server).forEach(key => {
    if (key in settingsStore.serverSettings) {
      formData.value.server[key] = settingsStore.serverSettings[key];
    }
  });

  Object.keys(formData.value.frontend).forEach(key => {
    if (key in settingsStore.frontendSettings) {
      formData.value.frontend[key] = settingsStore.frontendSettings[key];
    }
  });
  ...
  ['mine', 'shared-with-me', 'shared-by-me'].forEach(tab => {
    if (settingsStore.columnsConfig[tab]) {
      Object.keys(formData.value.columns[tab]).forEach(key => {
        if (key in settingsStore.columnsConfig[tab]) {
          formData.value.columns[tab][key] = settingsStore.columnsConfig[tab][key];
        }
      });
    }
  });
...
:deep(.compact-input.el-input-number.is-controls-right .el-input-number__increase) { width: 20px; height: 13px; line-height: 13px; font-size: 9px; border-left: 1px solid #dcdfe6; }
