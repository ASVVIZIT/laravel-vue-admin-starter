<template>
  <div class="set-row" :class="{ 'set-frozen': frozen }">
    ...
    <el-input-number v-if="isFieldVisible('reps')"
                     v-model="localSet.reps" :min="0" :max="1000"
                     placeholder="Повторы" size="small" controls-position="right"
                     :disabled="frozen" @change="emitChange" />

    <el-input-number v-if="isFieldVisible('weight')"
                     v-model="localSet.weight" :min="0" :max="1000" :step="0.5"
                     placeholder="кг" size="small" controls-position="right"
                     :disabled="frozen" @change="emitChange">
      <template #suffix><span class="unit">кг</span></template>
    </el-input-number>

    <el-input-number v-if="isFieldVisible('duration')"
                     v-model="localSet.duration" :min="0" :max="86400"
                     placeholder="сек" size="small" controls-position="right"
                     :disabled="frozen" @change="emitChange">
      <template #suffix><span class="unit">сек</span></template>
    </el-input-number>

    <el-input-number v-if="isFieldVisible('distance')"
                     v-model="localSet.distance" :min="0" :max="42195" :step="0.1"
                     placeholder="м" size="small" controls-position="right"
                     :disabled="frozen" @change="emitChange">
      <template #suffix><span class="unit">м</span></template>
    </el-input-number>

    <el-input v-model="localSet.notes" placeholder="Заметки" size="small"
              maxlength="50" show-word-limit :disabled="frozen" @input="emitChange" />

    <el-button v-if="removable && !frozen" type="danger" link size="small"
               @click="$emit('remove', index)" title="Удалить подход">
