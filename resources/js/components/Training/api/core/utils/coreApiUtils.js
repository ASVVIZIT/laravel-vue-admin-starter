/**
 * ============================================================================
 * API UTILS — ОБЩИЕ УТИЛИТЫ API (МОДУЛЬ TRAINING)
 * ============================================================================
 * 📁 Путь: @/components/Training/api/core/utils/coreApiUtils.js
 * ✅ Используется: Все API ресурсы модуля Training
...
export class CoreApiUtils {
    ...
    static logApiResponseUtils(method, url, response, requestId = null) {
        if (coreApiContextUtils.shouldLog('debug')) {
            logResponseUtils('CoreApiUtils', requestId, response.status || 200, 0, { data: response.data })
        }
    }
    ...
    static validateResponseUtils(response) {
        ...
        // Если ответ уже распаршен (response.data), проверяем его
        const data = response.data || response
        if (data.success === false) return { valid: false, error: data.message || 'Ошибка API' }
        return { valid: true }
    }
