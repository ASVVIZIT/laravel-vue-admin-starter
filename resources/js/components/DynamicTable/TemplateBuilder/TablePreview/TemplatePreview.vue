...
<template>
  <div class="template-preview">
    <div class="preview-header">
      ...
      <div class="test-mode-controls" v-if="showTestControls">
 ...
 * @emits {Event} column-order-update - Событие обновления порядка колонок
...
import PreviewCell from '@/components/DynamicTable/TemplateBuilder/cells/PreviewCell.vue';
import { usePreviewStore } from '@/components/DynamicTable/stores/previewStore';
...
// === ИСПРАВЛЕНИЕ: Добавлен ref для previewTableHeader ===
...
// === ИСПРАВЛЕНИЕ: Инициализация хранилища ===
...
const handleBodyScroll = (event) => {
  // === ИСПРАВЛЕНИЕ: Проверяем previewTableHeader.value перед использованием ===
...
// === ИСПРАВЛЕНИЕ: Обновленный onTestModeChange с правильной логикой ===
...
// === Lifecycle & Watchers ===
onMounted(async () => {
  console.log('[TemplatePreview] Component mounted');
  try {
    loadingReferenceTypes.value = true;

    const types = await dataSource.getReferenceTypes();
    console.log('[TemplatePreview] Types loaded:', types);
...
watch(() => props.columns, (newVal) => {
  console.log('[TemplatePreview.watch.props.columns] Columns changed:', newVal);
  ...
  console.log('[TemplatePreview.watch.props.columns] columnWidths.value SET:', columnWidths.value);
  console.log('[TemplatePreview.watch.props.columns] FINISHED');
}, { deep: true });

watch(() => props.selectedColumnIndex, (newIndex) => {
  console.log(`[TemplatePreview.watch.props.selectedColumnIndex] Changed from ${selectedPreviewColumnIndex.value} to ${newIndex}`);
  selectedPreviewColumnIndex.value = newIndex;
  console.log(`[TemplatePreview.watch.props.selectedColumnIndex] selectedPreviewColumnIndex.value SET to ${newIndex}`);
  console.log(`[TemplatePreview.watch.props.selectedColumnIndex] FINISHED`);
});

watch(() => props.selectedRowIndex, (newIndex) => {
  console.log(`[TemplatePreview.watch.props.selectedRowIndex] Changed from ${selectedPreviewRowIndex.value} to ${newIndex}`);
  selectedPreviewRowIndex.value = newIndex;
  console.log(`[TemplatePreview.watch.props.selectedRowIndex] selectedPreviewRowIndex.value SET to ${newIndex}`);
  console.log(`[TemplatePreview.watch.props.selectedRowIndex] FINISHED`);
});

// === ИСПРАВЛЕНИЕ: Watch для testMode из хранилища ===
watch(() => previewStore.testMode, (newMode) => {
  console.log(`[TemplatePreview.watch.previewStore.testMode] Store mode changed to: ${newMode}`);
  localTestMode.value = newMode;
  console.log(`[TemplatePreview.watch.previewStore.testMode] localTestMode.value SET to ${newMode}`);
  console.log(`[TemplatePreview.watch.previewStore.testMode] FINISHED`);
});
// === КОНЕЦ ИСПРАВЛЕНИЯ ===
// === Конец Lifecycle & Watchers ===

// === НОВОЕ: Методы для работы со справочниками ===
const loadReferenceData = async (entityType) => {
  ...
  if (loadingReference.value.has(entityType)) {
    console.log(`[TemplatePreview] Reference data for ${entityType} is already loading, skipping...`);
    return;
  }

  // Проверяем, есть ли уже данные
  if (referenceData.value[entityType] && referenceData.value[entityType].length > 0) {
    console.log(`[TemplatePreview] Reference data for ${entityType} already loaded, skipping...`);
    return;
  }
  ...
  try {
    ...
    console.error(`[TemplatePreview] Ошибка загрузки данных справочника ${entityType}:`, error);
