<template>
  <div class="i18n-lang-switcher-wrapper" @click.stop @mousedown.stop>
    <el-dropdown
        trigger="click"
        @command="handleSetLanguage"
        :teleported="false"
    >
      <button type="button" class="i18n-lang-switcher-btn" @click.stop>
...
.i18n-lang-switcher-wrapper { position: relative; display: inline-flex; }
.i18n-lang-switcher-btn {
  display: inline-flex; align-items: center; gap: 4px; padding: 2px 8px; height: 24px;
  background: #f5f7fa; border: 1px solid #dcdfe6; border-radius: 4px; cursor: pointer;
  font-size: 11px; font-weight: 500; color: #606266; transition: all 0.2s;
  &:hover { background: #ecf5ff; border-color: #b3d8ff; color: #1890ff; }
  .i18n-lang-switcher-current { line-height: 1; }
  .i18n-lang-switcher-arrow { font-size: 10px; margin-left: 2px; }
}
.i18n-lang-option { display: flex; align-items: center; gap: 8px; min-width: 120px;
...
.i18n-lang-toast {
  ...
  font-size: 11px; font-weight: 500; color: #52c41a; white-space: nowrap;
  z-index: 10; box-shadow: 0 2px 8px rgba(0, 0, 0, 0.08); pointer-events: none;
  .i18n-lang-toast-icon { font-size: 13px; color: #52c41a; }
}
.i18n-toast-fade-enter-active, .i18n-toast-fade-leave-active { transition: opacity 0.25s ease, transform 0.25s ease; }
.i18n-toast-fade-enter-from, .i18n-toast-fade-leave-to { opacity: 0; transform: translateX(-50%) translateY(-4px); }
.i18n-toast-fade-enter-to, .i18n-toast-fade-leave-from { opacity: 1; transform: translateX(-50%) translateY(0); }
</style>
