<template>
  <div className="battery-scene-container" ref="container"></div>
</template>
...
// Эффект жидкости
const applyFluidEffect = () => {
  if (!batteryFill || !batteryCritical) return;

  const time = Date.now() * 0.001;
  const waveHeight = Math.sin(time) * 0.02;

  // Нормальное заполнение
  batteryFill.position.x = waveHeight * 0.1;
  batteryFill.material.emissive = new THREE.Color(batteryColor.value);
  batteryFill.material.emissiveIntensity = 0.5;

  // Критическое заполнение
  batteryCritical.position.x = waveHeight * 0.1;
  batteryCritical.material.emissive = new THREE.Color(batteryColor.value);
  batteryCritical.material.emissiveIntensity = 0.5;
};
...
const animate = () => {
  ...
  // Продолжаем обновлять эффект жидкости
  if (props.fluidEffect) {
    applyFluidEffect();
  }
};
