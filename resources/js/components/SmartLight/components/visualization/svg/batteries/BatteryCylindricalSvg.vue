<template>
  <div class="battery-cylindrical-svg">
    ...
    <!-- Специфичные детали цилиндрической батареи -->
...
// ✅ Динамический поиск конфига по переданному ID
const config = computed(() => configStore.getBatteryConfigStore(props.batteryTypeId));

// ✅ Получение параметров из specs, fallback на дефолты
...
const shortName = computed(() => config.value?.shortName ?? 'AA');
const capacity = computed(() => config.value?.specs?.capacity ?? 2000);
const chemistry = computed(() => config.value?.specs?.chemistry?.toUpperCase() ?? '');
