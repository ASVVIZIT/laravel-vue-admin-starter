...
const props = defineProps({
  ...
  batteryTypeId: { type: String, default: 'li-ion-16340' },
...
const scale = computed(() => config.value?.visualConfig?.scale ?? 0.8)
const shortName = computed(() => config.value?.shortName ?? '16340')
const capacity = computed(() => config.value?.specs?.capacity ?? 880)
