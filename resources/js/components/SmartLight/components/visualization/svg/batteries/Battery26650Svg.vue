...
const props = defineProps({
  ...
  batteryTypeId: { type: String, default: 'li-ion-26650' },
...
const scale = computed(() => config.value?.visualConfig?.scale ?? 1.3)
const shortName = computed(() => config.value?.shortName ?? '26650')
const capacity = computed(() => config.value?.specs?.capacity ?? 5500)
