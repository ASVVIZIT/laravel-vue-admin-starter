...
const props = defineProps({
  ...
  batteryTypeId: { type: String, default: 'li-ion-10440' },
...
const scale = computed(() => config.value?.visualConfig?.scale ?? 0.75)
const shortName = computed(() => config.value?.shortName ?? '10440')
const capacity = computed(() => config.value?.specs?.capacity ?? 350)
