...
const props = defineProps({
  ...
  batteryTypeId: {type: String, default: 'li-ion-14500'},
...
const scale = computed(() => config.value?.visualConfig?.scale ?? 0.95)
const shortName = computed(() => config.value?.shortName ?? '14500')
const capacity = computed(() => config.value?.specs?.capacity ?? 900)
