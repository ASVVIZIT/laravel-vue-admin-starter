...
const saveSettings = async () => {
  console.log('[GlobalSettingsForm] saveSettings called');
  loading.value = true;
  try {
    const response = await store.settingsUpdateGlobalSettings(settings.value);
    console.log('[GlobalSettingsForm] API response:', response);
    if (response.success) {
      console.log('[GlobalSettingsForm] emit saved');
      emit('saved', settings.value);
    }
  } catch (error) {
    console.log('[GlobalSettingsForm] emit cancelled', error);
...
const handleSaved = () => {
  console.log('[GlobalSettingsModal] handleSaved called');
...
// ✅ ВАЖНО — ЭКСПОРТ МЕТОДОВ ДЛЯ РОДИТЕЛЯ
defineExpose({
  saveSettings,
  resetSettings,
  settings
