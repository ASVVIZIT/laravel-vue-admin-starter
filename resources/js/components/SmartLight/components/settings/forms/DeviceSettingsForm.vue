<template>
  <div class="device-settings-form">
    ...
    <div class="form-header">
      <span class="device-name" :title="device?.name">
        {{ device?.name || 'Устройство' }}
      </span>
      <el-tag
          :type="device?.is_fake ? 'warning' : 'success'"
          effect="plain"
          size="small"
          class="device-type-badge"
      >
        {{ device?.is_fake ? '🎭 Тест' : '🔌 Реал' }}
      </el-tag>
    </div>
    ...
    <div class="form-scroll-wrapper">
      ...
      >
        ...
        <el-form-item
            label="Крит. напряжение (В)"
            :error="validationErrors?.critical_voltage?.[0]"
            class="compact-item"
        >
          <el-input-number
              v-model="localSettings.critical_voltage"
              :min="voltageRules.min"
              :max="voltageRules.max"
              :step="voltageRules.step"
              :precision="2"
              :disabled="!voltageRules.enabled"
              controls-position="right"
              class="full-width compact-input"
              ref="criticalVoltageInput"
              @change="(val) => handleFieldChange('critical_voltage', val, voltageRules.min, voltageRules.max)"
              @input="clearFieldError('critical_voltage')"
          />
          <div class="form-tip" v-if="voltageRules.enabled">
            Минимальное напряжение для автоматического отключения
          </div>
          <div class="form-tip disabled-tip" v-else>
            Не применимо для текущего типа батареи
          </div>
        </el-form-item>

        <!-- 😴 БЛОК 2: Спящий режим платы -->
        <el-divider content-position="left" class="compact-divider">
          <el-icon><Moon /></el-icon> Спящий режим платы
        </el-divider>

        <el-form-item
            label="Интервал сна (сек)"
            :error="validationErrors?.sleep_interval?.[0]"
            class="compact-item"
        >
          <el-input-number
          ...
          />
          <div class="form-tip">
            Как часто плата просыпается для проверки напряжения. Не влияет на работу света.
          </div>
        </el-form-item>

        <el-form-item
            label="Аварийный интервал (сек)"
            :error="validationErrors?.emergency_sleep_interval?.[0]"
            class="compact-item"
        >
          <el-input-number
          ...
          />
          <div class="form-tip">
            Интервал сна при критическом разряде батареи
          </div>
        </el-form-item>

        <!-- ⚙️ БЛОК 3: Типы оборудования -->
        <el-divider content-position="left" class="compact-divider">
          <el-icon><Setting /></el-icon> Типы оборудования
        </el-divider>

        <el-form-item label="Тип батареи" class="compact-item">
          <el-select
              v-model="localSettings.battery_type_id"
              class="full-width compact-select"
              :loading="!typesStore.typesLoaded"
              :disabled="!typesStore.typesLoaded"
              size="small"
        ...
        <el-form-item label="Тип лампы" class="compact-item">
          <el-select
              v-model="localSettings.bulb_type_id"
              class="full-width compact-select"
              :loading="!typesStore.typesLoaded"
              :disabled="!typesStore.typesLoaded"
              size="small"
        ...
        <el-form-item label="Источник питания" class="compact-item">
          <el-select
              v-model="localSettings.power_supply_id"
              class="full-width compact-select"
              :loading="!typesStore.typesLoaded"
              :disabled="!typesStore.typesLoaded"
              size="small"
        ...
        <el-divider content-position="left" class="compact-divider">
          <el-icon><Connection /></el-icon> Группировка батарей
        </el-divider>

        <el-form-item label="Включить группировку" class="compact-item">
          <el-switch
        ...
        <el-form-item
            v-if="localSettings.battery_group_config.enabled"
            label="Тип соединения"
            :error="validationErrors?.['battery_group_config.type']?.[0]"
            class="compact-item"
        >
          <el-select
              v-model="localSettings.battery_group_config.type"
              class="full-width compact-select"
              size="small"
        ...
        <el-form-item
            v-if="localSettings.battery_group_config.enabled"
            label="Количество"
            :error="validationErrors?.['battery_group_config.count']?.[0]"
            class="compact-item"
        >
          <el-input-number
        ...
        <!-- ⚡ БЛОК 5: Дополнительные настройки -->
        <el-divider content-position="left" class="compact-divider">
          <el-icon><Cpu /></el-icon> Дополнительно
        </el-divider>

        <el-form-item label="Ток потребления (мА)" class="compact-item">
          <el-input-number
              v-model="localSettings.custom_consumption_mA"
              :min="1"
              :max="1000"
              :step="1"
              controls-position="right"
          ...
          />
          <div class="form-tip">
            Потребление устройства в мА (для точного расчёта времени работы)
          </div>
        </el-form-item>
        ...
        <el-form-item class="compact-actions">
          <el-button
              ...
              @click="handleSave"
              :loading="loading"
              class="full-width compact-button"
              size="small"
          >
            Сохранить настройки
          </el-button>
...
import { Lightning, Moon, Setting, Connection, Cpu } from '@element-plus/icons-vue';
...
// === LOCAL STATE ===
// ✅ НОВЫЙ ФЛАГ: защита от перезаписи при поллинге
...
const powerSupplyTypes = computed(() => typesStore.powerSupplyTypesForDropdownStore || []);
...
watch(
    () => props.device,
    (newDevice) => {
      // ✅ ЗАЩИТА: не перезаписываем, если пользователь редактирует
      if (isEditing.value) return;
      if (!newDevice?.form_settings) return;

      // Инициализация ВСЕХ полей из form_settings
      localSettings.value = {
        ...
        battery_group_config: newDevice.form_settings.battery_group_config ?? {
          enabled: false,
          type: 'series',
          count: 1
        },
        custom_consumption_mA: newDevice.form_settings.custom_consumption_mA ?? 40,
        ...Object.fromEntries(
            Object.entries(newDevice.form_settings).filter(
                ([key]) => ![
                  ...
                  'battery_group_config', 'custom_consumption_mA'
                ].includes(key)
            )
        )
      };
...
watch(
    () => localSettings.value.battery_type_id,
    (newTypeId) => {
      const battery = typesStore.getBatteryTypeByIdStore(newTypeId);
      if (battery?.specs) {
        triggerBoundaryHint('critical_voltage', battery.specs.minVoltage, battery.specs.maxVoltage);
      }
    }
);
...
const handleFieldChange = (field, val, min, max) => {
  // ✅ ПОЛЬЗОВАТЕЛЬ НАЧАЛ РЕДАКТИРОВАНИЕ — включаем защиту
...
const onBatteryTypeChange = (newTypeId) => {
  // ✅ ПОЛЬЗОВАТЕЛЬ ВЫБРАЛ ТИП — включаем защиту
  isEditing.value = true;
  const battery = typesStore.getBatteryTypeByIdStore(newTypeId);
  if (battery?.specs) {
    if (localSettings.value.critical_voltage < battery.specs.minVoltage) {
      localSettings.value.critical_voltage = battery.specs.minVoltage;
    }
    if (localSettings.value.critical_voltage > battery.specs.maxVoltage) {
      localSettings.value.critical_voltage = battery.specs.maxVoltage;
    }
  }
};

const handleGroupConfigChange = (enabled) => {
  // ✅ ПОЛЬЗОВАТЕЛЬ ИЗМЕНИЛ ГРУППИРОВКУ — включаем защиту
  isEditing.value = true;
  if (!enabled) {
...
// ✅ ИСПРАВЛЕНО: Корректная структура payload + сброс флага isEditing после сохранения
const handleSave = async () => {
  loading.value = true;
  validationErrors.value = {};

  try {
    const validation = validateDeviceSettingsUtils(
    ...
    if (!validation.valid) {
      validationErrors.value = normalizeValidationErrorsUtils(
          validation.errors.reduce((acc, err, idx) => {
            const fieldMatch = err.match(/(?:Критическое напряжение|Интервал сна|Аварийный интервал|Тип аккумулятора|Тип лампы|Количество аккумуляторов|Ток потребления)/i);
            const field = fieldMatch
                ? fieldMatch[0].toLowerCase().replace(/[^a-zа-яё0-9]/g, '_')
                : `field_${idx}`;
            acc[field] = [err];
            return acc;
          }, {})
    ...
    // Структурируем payload: поля БД отдельно, настройки в JSON
    const payload = {
      ...
      power_supply_id: localSettings.value.power_supply_id,
      battery_group_config: localSettings.value.battery_group_config,
      settings: {
        custom_consumption_mA: localSettings.value.custom_consumption_mA,
    ...
    if (response?.success) {
      ElNotification({
        title: 'Успех',
        message: 'Настройки сохранены на сервере',
        type: 'success',
        duration: 2000
      });
...
// Emit при изменении локальных настроек (для предпросмотра)
...
:deep(.compact-input .el-input__wrapper),
...
:deep(.compact-input.el-input-number.is-controls-right .el-input__wrapper) {
  padding-right: 26px;
}

:deep(.compact-select .el-select-dropdown) {
  font-size: 10px;
  padding: 4px 0;
}

:deep(.compact-select .el-select-dropdown__item) {
  font-size: 10px;
  padding: 6px 12px;
}

:deep(.compact-item .el-switch) {
  transform: scale(0.85);
  margin-left: 2px;
}

:deep(.compact-item .el-switch__core) {
  width: 28px;
  height: 14px;
}

:deep(.compact-item .el-switch__label) {
  font-size: 8px;
}
...
:deep(.compact-actions .el-button) {
  font-size: 10px;
  padding: 4px 8px;
  height: 24px;
}
