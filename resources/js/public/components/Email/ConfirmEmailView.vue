<template>
  <div class="auth-standalone-page">
    <div class="auth-card">
      ...
      <div class="footer">
        <p class="footer-text">
          Вопросы? <a href="mailto:DillerASV@yandex.ru">DillerASV@yandex.ru</a>
        </p>
      </div>
    </div>
  </div>
...
const title = computed(() => {
  if (status.value === 'loading') return 'Подтверждение...'
  if (status.value === 'success') return 'Email успешно подтверждён!'
  return 'Ошибка подтверждения'
})

const message = computed(() => {
  if (status.value === 'loading') return 'Пожалуйста, подождите, идёт проверка ссылки...'
  return serverMessage.value || (
      status.value === 'success'
          ? 'Ваш email подтверждён. Теперь вы можете войти в систему.'
...
const confirmEmail = async () => {
  ...
  try {
    ...
    status.value = 'error'
    serverMessage.value = error.response?.data?.message || 'Ссылка недействительна или срок её действия истёк.'
  }
}
