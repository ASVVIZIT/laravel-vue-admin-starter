...
const title = computed(() => {
  if (status.value === 'loading') return t('auth.restoreAccount.processingTitle') || 'Восстановление...'
  if (status.value === 'success') return t('auth.restoreAccount.successTitle') || 'Аккаунт восстановлен!'
  return t('auth.restoreAccount.errorTitle') || 'Ошибка восстановления'
})

const message = computed(() => {
  if (status.value === 'loading') return t('auth.restoreAccount.processingMessage') || 'Идёт восстановление вашего аккаунта...'
  return serverMessage.value || (
      status.value === 'success'
          ? (t('auth.restoreAccount.successMessage') || 'Ваш аккаунт успешно восстановлен. Теперь вы можете войти.')
          : (t('auth.restoreAccount.errorMessage') || 'Ссылка недействительна или аккаунт не найден.')
  )
})
...
onMounted(async () => {
  ...
  try {
    const res = await request({ url: `/auth/restore/${id}/${hash}`, method: 'get' })
    status.value = 'success'
