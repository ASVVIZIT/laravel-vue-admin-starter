...
const title = computed(() => {
  if (status.value === 'loading') return t('auth.emailVerify.processingTitle') || 'Проверка email...'
  if (status.value === 'success') return t('auth.emailVerify.successTitle') || 'Email подтверждён!'
  return t('auth.emailVerify.errorTitle') || 'Ошибка подтверждения'
})

const message = computed(() => {
  if (status.value === 'loading') return t('auth.emailVerify.processingMessage') || 'Идёт подтверждение вашего адреса...'
  return serverMessage.value || (
      status.value === 'success'
          ? (t('auth.emailVerify.successMessage') || 'Ваш email подтверждён. Теперь вы можете войти.')
          : (t('auth.emailVerify.errorMessage') || 'Ссылка недействительна или срок её действия истёк.')
  )
})
...
onMounted(async () => {
  ...
  try {
    const res = await request({ url: `/auth/verify-email/${id}/${hash}`, method: 'get' })
    status.value = 'success'
